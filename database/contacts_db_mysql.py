...
class Database:
    ...
    @classmethod
    def get_conn(cls) -> mysql.connector.MySQLConnection:
        if not cls.conn or not cls.conn.is_connected():
    ...
    @classmethod
    def create_contacts_mysql_table(cls):
        """创建联系人表"""
        conn = cls.get_conn()
        cursor = conn.cursor()
        try:
            ...
            logger.error(f"创建联系人表失败: {str(e)}")
        finally:
            cursor.close()

    @classmethod
    def get_contacts_from_mysql(cls, offset: Optional[int] = None, limit: Optional[int] = None) -> List[Dict]:
        """从数据库获取联系人，支持分页
    ...
    @classmethod
    def save_contacts_to_mysql(cls, contacts: List[Dict]) -> bool:
        """保存联系人列表到数据库"""
        try:
            conn = cls.get_conn()
            cursor = conn.cursor()
            # 创建表（如果不存在）
            cls.create_contacts_mysql_table()
    ...
    @classmethod
    def update_contact_in_mysql(cls, contact: Dict) -> bool:
        """更新单个联系人信息"""
        wxid = contact.get("wxid", "")
        if not wxid:
        ...
        try:
            ...
            # 检查联系人是否存在
            cursor.execute("SELECT wxid FROM contacts WHERE wxid = %s", (wxid,))
    ...
    @classmethod
    def get_contact_from_mysql(cls, wxid: str) -> Optional[Dict]:
        """从数据库获取单个联系人信息"""
        conn = cls.get_conn()
        cursor = conn.cursor(dictionary=True)
        try:
            # 查询联系人
            cursor.execute("SELECT * FROM contacts WHERE wxid = %s", (wxid,))
            row = cursor.fetchone()
            ...
            logger.error(f"从数据库获取单个联系人信息失败: {str(e)}")
            return None
        finally:
            cursor.close()

    @classmethod
    def delete_contact_from_mysql(cls, wxid: str) -> bool:
        """从数据库删除联系人"""
        try:
            conn = cls.get_conn()
            cursor = conn.cursor()
            # 删除联系人
            cursor.execute("DELETE FROM contacts WHERE wxid = %s", (wxid,))
            conn.commit()
    ...
    @classmethod
    def get_contacts_count_mysql(cls) -> int:
        """获取数据库中联系人数量"""
        try:
            conn = cls.get_conn()
            cursor = conn.cursor()
    ...
    @classmethod
    def get_all_contacts(cls) -> List[Dict]:
        """获取数据库中所有联系人
        ...
        return cls.get_contacts_from_mysql()