...
class GroupChatSummary(PluginBase):
    """
    ...
    def __init__(self):
        ...
        try:
            with open("plugins/GroupChatSummary/config.toml", "rb") as f:
                config = tomllib.load(f)

            plugin_config = config["GroupChatSummary"]
            ...
            logger.info("GroupChatSummary 插件配置加载成功")
        except FileNotFoundError:
            logger.error("config.toml 配置文件未找到，插件已禁用。")
            self.enable = False
        except Exception as e:
            logger.exception(f"GroupChatSummary 插件初始化失败: {e}")
            self.enable = False
    ...
    async def close(self):
        """插件关闭时，关闭相关资源"""
        logger.info("Closing GroupChatSummary plugin")
        ...
        logger.info("GroupChatSummary plugin closed")
